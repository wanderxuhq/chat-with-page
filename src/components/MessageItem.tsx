...
export default React.memo(MessageItem, (prev, next) => {
  ...
  // Check editing state
  const prevIsEditing = prev.editingId === prev.message.id;
  const nextIsEditing = next.editingId === next.message.id;
  if (prevIsEditing !== nextIsEditing) return false;
  if (nextIsEditing && prev.editContent !== next.editContent) return false;
  
  // Check copied state
  const prevIsCopied = prev.copiedId === prev.message.id;
  const nextIsCopied = next.copiedId === next.message.id;
  if (prevIsCopied !== nextIsCopied) return false;
  
  // Handlers
  if (prev.setEditingId !== next.setEditingId) return false;
  if (prev.setEditContent !== next.setEditContent) return false;
  if (prev.setCopiedId !== next.setCopiedId) return false;
  if (prev.handleSaveEdit !== next.handleSaveEdit) return false;
  if (prev.handleCancelEdit !== next.handleCancelEdit) return false;
